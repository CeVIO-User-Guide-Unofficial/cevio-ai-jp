 ...
 **PC不調によるインストール失敗** 
 ...
 PCを再起動後、Windowsの「設定」の「アプリ」の「インストールされているアプリ」で「CeVIO AI」やボイスを選んで「変更」から修復インストールを行ってください。修復で直らない場合は、「インストールされているアプリ」で「CeVIO AI」とボイスを一旦アンインストールし、再インストールを行ってください。
 ...
 **インストール先のファイルを直接削除や移動すると、その後のインストールやアンインストール、ソフトの動作に問題が生じます。** 
 ...
 のトラブルシューティングツールで、アンインストールできない「CeVIO AI」やボイスをアンインストールしてださい。
