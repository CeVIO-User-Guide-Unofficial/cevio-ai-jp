...
======
 ...
 『CeVIO AI』最新バージョン9.0.1.0 インストーラー（74MB）
----------------------------------------



 現在、手動アップデートのみの先行公開中です。
...
[![CeVIO_AI_Setup_Download](image/dl_bt.png)](https://cevio.jp/storage/cevio_ai/CeVIO_AI_Setup_x64_(9.0.1.0).msi)
...
[「可不(KAFU)」インストーラー アップデート](home/kafu_update) 
 ...
 2023/12/25 先行アップデート（バージョン9.0.1.0）
-----------------------------------


 ※音声波形表示やフリーアクセントモードなど、バージョン9.0の主な変更点は
 [2023/12/5 アップデート](#バージョン8.7.0.0) 
 をご覧ください。
   

 ※先行公開バージョン8.7.0.0～9.0.1.0までは、近日「バージョン9.0」として正式公開予定です。
...
**▼ソング関連** 
 ...
 ・音符単位のタイミング調整時、音符内最後の音素以外のタイミングは平行移動するように。
...
> 
>  ※[Shift]+ドラッグで、以前の音符内タイミング比率維持も可能。
>    
> 
> 
 ...
 ・楽譜編集画面で再生して調整画面に切り替えて停止するとエラーが発生することがあった不具合を修正。
   


 2023/12/21 先行アップデート（バージョン9.0.0.0）
-----------------------------------
...
**▼ソング関連** 
 ...
 ・音声波形取得中にソングトラック→オーディオトラック→ソングトラックと切り替えたり、ミュートトラックで再生→停止→再生と行うと、一時的に操作不能になることがあったのを修正。
   

  

 ・歌詞の変更後にアンドゥ→リドゥと行うと、タイミング調整が崩れることがあった不具合を修正。
...
> 
 ...
 2023/12/15 先行アップデート（バージョン8.7.3.0）
-----------------------------------
...
> 
 ...
 2023/12/12 先行アップデート（バージョン8.7.2.0）
-----------------------------------
...
**▼ソング関連** 
 ...
 ・音声波形表示中かつビブラート表示中に調整するとウェイトカーソルが表示され、もたつくのを修正。
   

  

 ・ソングのキャスト変更を繰り返すと強制終了することがあった不具合を修正。
   

  

 ・トークトラック選択状態でソングのキャストを変更したとき、キャスト画像が変わらないことがあったのを修正。
...
**▼その他** 
 ...
 2023/12/7 先行アップデート（バージョン8.7.1.0）
----------------------------------
...
**▼ソング関連** 
 ...
 ・音声波形表示中の再生で一時的に操作不能になったり、途中で再生が止まる不具合を修正。
...
**▼その他** 
 ...
 ・マウスカーソルがスクロールバー上にあるとき、ホイールスクロールはスクロールバーの水平／垂直に従うように。
...
2023/12/5 先行アップデート（バージョン8.7.0.0）
----------------------------------

 ※先行公開バージョン8.7.0.0～9.0.0.0までは、近日「バージョン9.0」として正式公開予定です。
...
> 
>  ・「ソング」メニューの「その他の表示」や右クリックメニューの「表示」からオン／オフ可能。
>    
> 
>  ・表示中はCPU・メモリ負荷が高まるため、動作が重い場合はオフに。
>    
> 
>  ・VOL調整画面で「声の元になる声帯振動の大きさ」を調整して、音声波形で聴感音量を確認できます。
>    
> 
> 
...
> 
 ...
 ・MIDIインポートウィンドウに「歌詞の文字コード」を追加。「UTF-8」も選択可能に。
...
> 
 ...
 ・声質の調整画面で、既定値を消しゴムでドラッグすると調整値（オレンジ色）になる不具合を修正。
...
> 
>  ※他の画面も含め、[Shift]+ドラッグでクオンタイズ基準とタイミング基準を切替可能。
>    
> 
